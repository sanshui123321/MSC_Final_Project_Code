...
REFERENCE_IMAGE="/home/tao/test_code_tao_blocked/screenshot/furbo.png" # Reference image
Para_file="/home/tao/test_code_tao_blocked/furbo.txt" # File containing the parameters
experiment_name='furbo_blocked' #slect from 'furbo','google_nest','withings','blink'
repeat_times=1
old_domain_name='product.furbo.co'
new_ip_address='192.168.0.1'


RESULT_FILE="/home/tao/test_code_tao_blocked/${experiment_name}_results.txt" # File for recording the results
PHONE="pixel3a" # The phone  ids/phone
SCREENSHOT_DIR="/home/tao/test_code_tao_blocked/screenshot" # Directory for screenshots
...
for ((i=0; i<repeat_times; i++)); do
    ...
    #Restart the device to delete the dns cache (tapo)
    adb shell -n monkey -p com.tplink.iot -c android.intent.category.LAUNCHER 1
    sleep 3
    adb shell -n input tap 440 1377
    sleep 30
    adb shell -n input tap 440 1377
    sleep 120


    # Start the dns_override
    cd /opt/moniotr
    /opt/moniotr/bin/dns-override.sh add "$mac_address" "$old_domain_name" "$new_ip_address"
    sleep 5
    ...
    # Stop the dns_override
    cd /opt/moniotr
    /opt/moniotr/bin/dns-override.sh del "$mac_address" "$old_domain_name"
    ...
    compare_result=$(python3 /home/tao/test_code_tao_blocked/compare_images.py "$SCREENSHOT_DIR/$SCREENSHOT_FILE" "$REFERENCE_IMAGE")
